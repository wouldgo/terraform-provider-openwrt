...
func (c *uci) GetAll(sections ...any) ([]System, error) {
	if len(sections) == 0 {
		return nil, fmt.Errorf("no sections specified")
	}

	result, err := call(*c.url, *c.token, "uci", "get_all", sections)
	if err != nil {
		return nil, err
	}
...
func (c *uci) GetSystem() (*System, error) {
	result, err := c.GetAll("system")
	if err != nil {
		return nil, err
	}
...
func (c *uci) TSet(data any, section ...any) error {
	data, err := purgeFields(&data)
	if err != nil {
		return err
	}
	section = append(section, data)
	_, err = call(*c.url, *c.token, "uci", "tset", section)
	return err
}

func (c *uci) Add(section ...any) (string, error) {
	raw, err := call(*c.url, *c.token, "uci", "add", section)
	if err != nil {
		return "", err
	}
	var s string
...
func (c *uci) Delete(section ...any) error {
	_, err := call(*c.url, *c.token, "uci", "delete", section)
	return err
}

func (c *uci) uciCommit(section ...any) error {
	resp, err := call(*c.url, *c.token, "uci", "commit", section)
	if err != nil {
...
func (c *uci) uciRevert(section ...any) error {
	resp, err := call(*c.url, *c.token, "uci", "revert", section)
	if err != nil {
