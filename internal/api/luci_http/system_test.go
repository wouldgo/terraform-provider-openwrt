...
func TestUnHappySystemRPCs(t *testing.T) {
	expectedHost,
		expectedToken,
		expectedUsername,
		expectedPassword := testutil.TestingData(t)
	expectedSection := "system"
	mockedRoundTripper := systemUnHappyPathMockedRoundTripper(
		t,
		expectedHost,
		expectedUsername,
		expectedPassword,
	...
	_, err = system.GetAll(t.Context())
	if err == nil || !errors.Is(err, luci.ErrSectionsNotSpecified) {
		t.Errorf("expected system get all in error with error %s, got %s", luci.ErrSectionsNotSpecified, err)
	}

	alreadyCancelledCtx, cancel := context.WithCancel(t.Context())
	cancel()
	_, err = system.GetSystem(alreadyCancelledCtx)
	if err == nil || !errors.Is(err, internal_http.ErrRpcTimeout) {
		t.Errorf("expected system get system in error with %s, got %s", internal_http.ErrRpcTimeout, err)
	}

	_, err = system.GetSystem(t.Context())
	if err == nil {
		t.Error("expected system get system in error")
	}

	err = system.TSet(t.Context(), "foo")
	if err == nil {
		t.Error("exptected system tset in error")
	}

	err = system.TSet(t.Context(), "foo", "bar")
	if err == nil {
		t.Error("exptected system tset in error")
	}

	_, err = system.Add(t.Context())
	if err == nil {
		t.Error("exptected system add in error")
	}

	err = system.Delete(t.Context())
	if err == nil {
		t.Error("exptected system delete in error")
	}

	err = system.CommitOrRevert(t.Context())
	if err == nil {
		t.Error("exptected system commit or revert in error")
	}
...
func systemUnHappyPathMockedRoundTripper(
	t *testing.T,
	expectedHost *url.URL,
	username,
	password,
