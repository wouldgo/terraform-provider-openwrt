...
func (s SystemResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	name, err := s.provider.Add("system", "system")
	if err != nil {
	...
	err = s.provider.TSet(plan, "system", plan.Id.ValueString())
	if err != nil {
		resp.Diagnostics.AddError(fmt.Sprintf("Failed to update config %q", plan.Id.ValueString()), err.Error())
		return
	}

	if errs := s.provider.CommitOrRevert("system", plan.Id.ValueString()); len(errs) > 0 {
...
func (s SystemResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	sm, err := s.provider.GetAll("system", state.Id.ValueString())
	if err != nil {
...
func (s SystemResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	sm, err := s.provider.GetAll(s.provider, "system", state.Id.ValueString())
	if err != nil {
	...
	err = s.provider.TSet(state, "system", state.Id.ValueString())
	if err != nil {
		resp.Diagnostics.AddError(fmt.Sprintf("Failed to update config %q", state.Id.ValueString()), err.Error())
		return
	}

	if errs := s.provider.CommitOrRevert("system", state.Id.ValueString()); len(errs) > 0 {
		for _, err := range errs {
			resp.Diagnostics.AddError("failed to commit or revert the change", err.Error())
		}
		return
...
func (s SystemResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	err := s.provider.Delete("system", state.Id.ValueString())
	if err != nil {
